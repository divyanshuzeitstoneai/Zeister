...
```graphql
query IngestOrdersForScoring($cursor: String) {
          ...
          edges {
            node {
              ...
              variant {
                ...
                dimensionsMetafield: metafield(namespace: "zeitster", key: "dimensions") {
                  value
                }
              }
            }
...
| Level | Namespace | Key | Type | Example JSON Payload |
|---|---|---|---|---|
| **Shop** | `zeitster` | `category_margin_targets` | `json` | `{"apparel": 0.50, "electronics": 0.40, "furniture": 0.35}` |
| **Shop** | `zeitster` | `discount_dependency_thresholds` | `json` | `{"healthy": 20, "warning": 30}` |
| **ProductVariant** | `zeitster` | `dimensions` | `json` | `{"length": 50, "width": 40, "height": 30, "unit": "cm"}` |

### External Ingestion Stubs (Carrier & Marketplace Tables)

#### `actual_shipping_costs` Ingestion Schema
```json
{
  "order_id": "gid://shopify/Order/7388106490095",
  "tracking_number": "TRACK123456",
  "carrier": "FedEx",
  "actual_courier_cost": 20.00,
  "currency": "USD"
}
```

#### `marketplace_channel_fees` Ingestion Schema
```json
{
  "order_id": "gid://shopify/Order/7388106490095",
  "channel": "amazon",
  "channel_fee_amount": 37.50,
  "currency": "USD"
}
```
